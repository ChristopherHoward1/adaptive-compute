...
Python 3.12 (`src/` layout) for the research code, plus the agentic-coding harness (Bash `set -uo pipefail` scripts + Markdown skills/agents) as the development machinery. No research functionality exists yet — the package is an empty scaffold. The implementer runtime is Codex (`config.yaml`); reviewers are Claude (`opus`) plus a Codex second reviewer. Profile: `machine-learning`.
...
- `src/adaptive_compute/` — the Python package. Currently only `__init__.py` (`__version__`); no research modules yet.
- `tests/` — `test_smoke.py` (package-import smoke test, run by pytest) and `test-scripts.sh` (the harness's shell smoke suite, run by the gate hook).
- `pyproject.toml` — package metadata + tool config: ruff (lint `E,F,I` + format), pytest (`pythonpath=["src"]`), mypy.
...
`bash scripts/gate.sh` — required-tool preflight (`shellcheck`), shellcheck over tracked `*.sh`, `ruff check .`, `pytest -q`, plus `gate.d/` hooks (`test-scripts.sh` shell smoke suite, `py-quality.sh` format+types, `ds-hygiene.sh`, `nb-clean.sh`). CI runs the gate on push. Currently green.
